...
class OtpApiView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user = userService.get_user(serializer.validated_data['user_id'])
            if serializer.validated_data['secret_id']:
                secret = secretService.get_secret_by_id(serializer.validated_data['secret_id'])
            else:
                secret = None
...
class VerifyOtpApiView(APIView):
    ...
    def post(self, request):
        ...
        if not user_id or not code:
            return Response({"message": "user_id and code are required"}, status=status.HTTP_400_BAD_REQUEST)
